...
* [!DNL Workfront] - **7 febbraio** - _Scopri dieci suggerimenti per la comunicazione per tenere traccia di te e dei tuoi utenti_

* [!DNL Campaign Classic] - **8 febbraio** - _Come T. Rowe Price spinge il coinvolgimento dei clienti_

* [!DNL Campaign Standard] - **9 febbraio** - Interruzione del caffè nella Comunità - _Adobe [!DNL Campaign] Interruzione caffè Q&amp;A nella community - APAC_

Per la registrazione degli eventi e un programma completo, vedi [Eventi sull&#39;Experience League](https://experienceleague.adobe.com/events/?lang=it).
...
| Funzione | Descrizione |
| ------- | ----------- |
| [!UICONTROL Campagne], [!UICONTROL Bulksheet], [!UICONTROL Advanced (ACM)] | (Versione del 5 gennaio; campagne [!DNL Google Ads] e [!DNL Microsoft Advertising]) Se hai creato personalizzatori di annunci negli editor del network pubblicitario, puoi utilizzarli come attributi nei campi di testo degli annunci di ricerca reattivi in [!DNL Advertising Search]. All’interno dei titoli degli annunci, delle descrizioni e del primo e secondo percorso degli URL di visualizzazione, utilizza i seguenti formati per includere un personalizzatore di annunci:<ul><li>[!DNL Google Ads]: {CUSTOMIZER.AdCustomizerName:DefaultText}</code></li><li>[!DNL Microsoft Advertising]: {CUSTOMIZER.Attribute name:default text}</code></li></ul>Esempio di un personalizzatore di annunci in un titolo di annuncio: “Vasto inventario di stivali da pioggia scontati del `{CUSTOMIZER.Discount:10%}`.” |
| [!UICONTROL Importa campagne] | (versione del 5 gennaio; [!DNL Google Ads] e [!DNL Microsoft Advertising] campagne) Puoi replicare le tue [!DNL Google Ads] personalizzatori di annunci per [!DNL Microsoft Advertising] campagne da [!UICONTROL Ricerca] > [!UICONTROL Strumenti] > [!UICONTROL Importa campagne]. Vengono replicati automaticamente se non personalizzi il processo di importazione. Se si personalizza il processo di importazione specificando gli elementi da importare, assicurarsi di selezionare [!UICONTROL Feed] > [!UICONTROL Feed di personalizzazione degli annunci.] |
